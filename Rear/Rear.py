"""
╔══════════════════════════════════════════════════════════════════════╗
║   Rear — Text File → build123d + OCP CAD Viewer                    ║
╠══════════════════════════════════════════════════════════════════════╣
║  SETUP (one-time)                                                   ║
...
║                                                                     ║
║  RUN                                                                ║
║    python Rear_from_txt.py                                          ║
║    python Rear_from_txt.py  /path/to/other.txt                     ║
║                                                                     ║
║  INPUT                                                              ║
║    /Users/softage/Desktop/gamebub_files/component files/           ║
║    Rear/Rear_vertices_triangles.txt                                 ║
║                                                                     ║
║  OUTPUT                                                             ║
║    /Users/softage/Desktop/gamebub_files/component files/Rear/      ║
║      Rear_rebuilt.stl                                              ║
║      Rear_rebuilt.step                                             ║
╠══════════════════════════════════════════════════════════════════════╣
║  PIPELINE                                                           ║
...
OUT_DIR  = "/Users/softage/Desktop/gamebub_files/component files/Rear"
STL_OUT  = os.path.join(OUT_DIR, "Rear_rebuilt.stl")
STEP_OUT = os.path.join(OUT_DIR, "Rear_rebuilt.step")
...
def parse_txt(path: str):
    ...
    Section 1 — vertex lines:
        <id>   <x>   <y>   <z>
        e.g.:  0   -30.639185   3.217774   139.500122
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"\n  Text file not found:\n  {path}\n\n"
            f"  Ensure Rear_vertices_triangles.txt exists at that path."
        )
...
def write_binary_stl(vertices: dict, triangles: list, path: str) -> int:
    ...
    with open(path, "wb") as f:
        header = b"Rebuilt from Rear_vertices_triangles.txt"
...
def main():
    print()
    print("┌" + "─" * 62 + "┐")
    print("│   Rear  —  Text File → build123d + OCP CAD Viewer        │")
    print("└" + "─" * 62 + "┘")
    ...
    # import_stl() returns a fully tessellated build123d Shape —
    # the only type that OCP CAD Viewer renders reliably.
    ...
    if OCP_AVAILABLE:
        try:
            # Full keyword-argument call (ocp_vscode >= 2.x)
            show(
                mesh,
                names        = ["Rear"],
                colors       = ["#c0392b"],   # deep red
                alphas       = [1.0],
                axes         = True,
                axes0        = True,
                grid         = (True, True, True),
            ...
            # Older ocp_vscode — try without keyword args
            try:
                show(mesh)
