"""
╔══════════════════════════════════════════════════════════════════════╗
║   DpadButton — Text File → build123d + OCP CAD Viewer              ║
╠══════════════════════════════════════════════════════════════════════╣
║  SETUP (one-time)                                                   ║
...
║                                                                     ║
║  RUN                                                                ║
║    python DpadButton_from_txt.py                                    ║
║    python DpadButton_from_txt.py  /path/to/other.txt               ║
║                                                                     ║
║  INPUT                                                              ║
║    .../component files/DpadButton/                                  ║
║    DpadButton_vertices_triangles.txt                                ║
║                                                                     ║
║  OUTPUT                                                             ║
║    .../component files/DpadButton/                                  ║
║      DpadButton_rebuilt.stl                                        ║
║      DpadButton_rebuilt.step                                       ║
╠══════════════════════════════════════════════════════════════════════╣
║  PIPELINE                                                           ║
...
TXT_FILE = (
    sys.argv[1] if len(sys.argv) > 1
         ...
         "DpadButton/DpadButton_vertices_triangles.txt"
)

OUT_DIR  = ("/Users/softage/Desktop/gamebub_files/component files/"
            "DpadButton")
STL_OUT  = os.path.join(OUT_DIR, "DpadButton_rebuilt.stl")
STEP_OUT = os.path.join(OUT_DIR, "DpadButton_rebuilt.step")
...
def parse_txt(path: str):
    ...
    Section 1 — vertex lines:
        <id>   <x>   <y>   <z>
        e.g.:  0   -19.460070   -5.857914   44.018578
...
def write_binary_stl(vertices: dict, triangles: list, path: str) -> int:
    ...
    with open(path, "wb") as f:
        f.write(b"Rebuilt from DpadButton_vertices_triangles.txt"
                .ljust(80, b"\x00"))
...
def main():
    print()
    print("┌" + "─" * 62 + "┐")
    print("│   DpadButton  —  Text File → OCP CAD Viewer              │")
    print("└" + "─" * 62 + "┘")
    ...
    if OCP_AVAILABLE:
        ...
        if not displayed:
            try:
                show_object(mesh,
                            name    = "DpadButton",
                            options = {"color": "#8e44ad", "alpha": 1.0})
